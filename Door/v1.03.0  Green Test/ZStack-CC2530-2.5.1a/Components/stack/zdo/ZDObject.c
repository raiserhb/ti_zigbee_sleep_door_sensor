...
{
  ZStatus_t ret;
  uint32 u32OrpChanal = 0;//add
  if((_NIB.nwkLogicalChannel >= 0x0B)&&(_NIB.nwkLogicalChannel <= 0x1A))
  {
    u32OrpChanal = ((uint32)1<<_NIB.nwkLogicalChannel);//add
    zgDefaultChannelList=u32OrpChanal;
  }
  ...
  if ( ZG_BUILD_JOINING_TYPE && (logicalType == NODETYPE_ROUTER || logicalType == NODETYPE_DEVICE) )
  {
    if ( (startMode == MODE_JOIN) || (startMode == MODE_REJOIN) )
    {
      ...
      if((_NIB.nwkLogicalChannel >= 0x0B)&&(_NIB.nwkLogicalChannel <= 0x1A))  //add
      {
        u32OrpChanal = ((uint32)1<<_NIB.nwkLogicalChannel);       //add
        zgDefaultChannelList=u32OrpChanal;    //add
      }
